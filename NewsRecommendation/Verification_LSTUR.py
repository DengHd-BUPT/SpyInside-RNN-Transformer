...
import importlib
from Abstraction_LSTUR import LSTURDP
...
def evaluate(model, directory, num_workers, max_count=sys.maxsize):
    ...
    user_dataset = UserDataset(path.join(directory, args.test_userlogs),
    ...
    for minibatch in tqdm(user_dataloader,
        ...
        # lb verification
        if any(user_string not in user2vector_lb for user_string in user_strings):
            clicked_news_vector = torch.stack([
                torch.stack([news2vector[x].to(device) for x in news_list],
                            dim=0) for news_list in minibatch["clicked_news"]
            ],
                                              dim=0).transpose(0, 1)
            # print(clicked_news_vector)
            user_vector = model.get_user_vector(
                minibatch['user'], minibatch['clicked_news_length'],
                clicked_news_vector)
            # lb verification
            for user, vector in zip(user_strings, user_vector.lb):
                if user not in user2vector_lb:
                    user2vector_lb[user] = vector
            minibatchend = time.time()
            # ub verification
            for user, vector in zip(user_strings, user_vector.ub):
                if user not in user2vector_ub:
                    user2vector_ub[user] = vector
            print("The running time of the minibatch is " + str(minibatchend - minibatchstart))
    ...
    for minibatch in tqdm(behaviors_dataloader,
        ...
        candidate_news_vector = torch.stack([
            ...
            for news in minibatch['impressions']
                                            ...
                                            dim=0)
        user_vector = user2vector_lb[minibatch['clicked_news_string'][0]]
...
if __name__ == '__main__':
    ...
    model = LSTURDP(config).to(device)
    all_checkpoints = {
